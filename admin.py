...
async def admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for b in bet_stats:
        ...
        bet_lines += (
            f"  • {candidate}: {b['total_bets']} bets (${b['total_staked']:.2f})\n"
        )
    if not bet_lines:
    ...
    keyboard = [
        ...
        [
            ...
            InlineKeyboardButton(
                "📜 All Withdrawals", callback_data="admin_all_withdrawals"
            ),
        ],
        [
...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "admin_refresh":
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    "📥 Pending Deposits", callback_data="admin_pending"
                ),
                InlineKeyboardButton(
                    "📋 All Deposits", callback_data="admin_all_deposits"
                ),
            ],
            [
                InlineKeyboardButton(
...
async def show_pending_deposits(query, context):
    ...
    for dep in deposits[:10]:
        msg = (
            f"📥 *Pending Deposit*\n\n"
            f"🆔 ID: `{dep['id']}`\n"
            f"👤 User: {esc(dep['full_name'])} (@{esc(dep['username'])})\n"
            f"💵 Amount: ${dep['amount_usd']:.2f} USD\n"
            f"💳 Method: {esc(dep['method'])}\n"
            f"🔗 TX: `{dep['tx_hash']}`\n"
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    "✅ Confirm", callback_data=f"confirm_dep_{dep['id']}"
                ),
                InlineKeyboardButton(
                    "❌ Reject", callback_data=f"reject_dep_{dep['id']}"
                ),
            ]
        ]
    ...
    await query.edit_message_text(
        f"📥 Showing {len(deposits[:10])} pending deposit(s)."
    )
...
async def show_all_deposits(query):
    ...
    for dep in deposits[:20]:
        status_emoji = {"confirmed": "✅", "pending": "⏳", "rejected": "❌"}.get(
            dep["status"], "⚪"
        )
        lines.append(
            f"{status_emoji} `{dep['id']}` — ${dep['amount_usd']:.2f} — "
            f"{esc(dep['method'])} — {esc(dep.get('username', 'N/A'))} — {dep['created_at'][:10]}"
        )
...
async def show_wallet_menu(query):
    ...
    msg = f"💳 *Wallet Addresses*\n\nUSDT TRC20:\n`{usdt}`\n\nSolana:\n`{sol}`"
    keyboard = [
        [
            InlineKeyboardButton(
                "✏️ Set USDT TRC20", callback_data="setwallet_USDT_TRC20"
            )
        ],
...
async def show_all_users(query):
    ...
    for u in users[:20]:
        lines.append(
            f"• {esc(u['full_name'])} (@{esc(u.get('username', 'N/A'))}) — "
...
async def show_all_bets(query):
    ...
    for b in bets[:20]:
        candidate = CANDIDATES.get(b["candidate"], {}).get("name", b["candidate"])
        lines.append(
            f"• {esc(b.get('username', 'N/A'))} — {esc(candidate)} — "
...
async def handle_reject_deposit(query, context, deposit_id: str):
    ...
    if not success:
        await query.edit_message_text(
            "❌ Could not reject deposit (already processed?)."
        )
        return

    await query.edit_message_text(
        f"❌ Deposit `{deposit_id}` rejected.", parse_mode="Markdown"
    )
...
async def setwallet_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(args) < 2:
        await update.message.reply_text(
            "Usage: `/setwallet <USDT_TRC20|SOLANA> <address>`", parse_mode="Markdown"
        )
        return
...
# ─── Wallet address conversation handler ──────────────────────────────────────
...
async def show_pending_withdrawals(query, context):
    ...
    await query.edit_message_text(
        f"💸 Sending {len(withdrawals[:10])} pending withdrawal(s)..."
    )
    ...
    for wd in withdrawals[:10]:
        ...
        keyboard = [
            [
                InlineKeyboardButton(
                    "✅ Approve", callback_data=f"approve_wd_{wd['id']}"
                ),
                InlineKeyboardButton(
                    "❌ Reject", callback_data=f"reject_wd_{wd['id']}"
                ),
            ]
        ]
        await context.bot.send_message(
